...
//   RC_UPGRADE_SIGNING_SECRET          - HMAC secret paired with the PRODUCTION
//                                        Supabase sign-upgrade-link signer.
//   RC_UPGRADE_SIGNING_SECRET_STAGING  - HMAC secret paired with the STAGING
//                                        Supabase signer. One Pages deployment
//                                        serves both envs, so the verifier
//                                        selects by the env param; the two
//                                        values MUST differ.
//   Both are server-only; NEVER in the RN bundle, NEVER committed. An unset
//   secret means every link for that env fails to verify -> fallback page.
//
...
function checkoutHtml({ uid, productId, apiKey, env, scheme, plan }) {
  ...
  const config = JSON.stringify({ uid, productId, apiKey, env, scheme, plan }).replace(
    /</g,
    "\\u003c"
  );
  return `<!DOCTYPE html>
...
<body>
  ...
  <script type="module" src="/subscribe-app.js?v=14"></script>
</body>
</html>`;
}
...
export async function onRequestGet(context) {
  ...
  // Per-environment HMAC secret. The staging and production Supabase signers
  // hold DIFFERENT secrets (security model in sign-upgrade-link/index.ts), and
  // this ONE Pages deployment serves both envs, so the verifier must select the
  // secret by the env param. No cross-env fallback: an unset secret fails
  // closed inside verifySignedLink and serves the fallback page.
  const signingSecret =
    env === "staging"
      ? context.env.RC_UPGRADE_SIGNING_SECRET_STAGING
      : context.env.RC_UPGRADE_SIGNING_SECRET;
